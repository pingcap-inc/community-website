...
export const getCaptchaToken = (recaptchaKey, action = 'submit') =>
  new Promise((resolve, reject) => {
    if (!window.grecaptcha) {
      reject(new Error('无法加载 reCAPTCHA'));
    }
    window.grecaptcha?.ready(() => {
      window.grecaptcha.execute(recaptchaKey, { action }).then(resolve).catch(reject);
    });
  });
